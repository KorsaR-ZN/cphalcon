...
PHP_METHOD(Phalcon_Di_Service_Builder, _buildParameters) {
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_1, &_0)
		...
		ZEPHIR_CALL_METHOD(&_3, this_ptr, "_buildparameter", &_4, 170, dependencyInjector, position, argument);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Service_Builder, build) {
	...
	if (Z_TYPE_P(parameters) == IS_ARRAY) {
		...
		if (zephir_array_isset_string_fetch(&arguments, definition, SS("arguments"), 0 TSRMLS_CC)) {
			ZEPHIR_CALL_METHOD(&_0, this_ptr, "_buildparameters", &_1, 171, dependencyInjector, arguments);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(instance);
	...
	if (zephir_array_isset_string_fetch(&paramCalls, definition, SS("properties"), 0 TSRMLS_CC)) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_9, &_8)
			...
			ZEPHIR_CALL_METHOD(&_0, this_ptr, "_buildparameter", &_12, 170, dependencyInjector, propertyPosition, propertyValue);
			zephir_check_call_status();
