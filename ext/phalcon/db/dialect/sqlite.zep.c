			...
			if (Z_TYPE_P(typeValues) == IS_ARRAY) {
				...
				for (
				  ...
				  ; zephir_hash_move_forward_ex(_5, &_4)
					...
					ZVAL_STRING(&_7, "\"", 0);
					ZEPHIR_CALL_FUNCTION(&_2, "addcslashes", &_8, 136, value, &_7);
					zephir_check_call_status();
				...
				ZVAL_STRING(&_10, "\"", 0);
				ZEPHIR_CALL_FUNCTION(&_11, "addcslashes", &_8, 136, typeValues, &_10);
				zephir_check_call_status();
				ZEPHIR_INIT_VAR(_12);
...
PHP_METHOD(Phalcon_Db_Dialect_Sqlite, addColumn) {
	...
	if (zephir_is_true(_4)) {
		...
		if (zephir_memnstr_str(_5, SL("CURRENT_TIMESTAMP"), "phalcon/db/dialect/sqlite.zep", 152)) {
			...
			ZEPHIR_SINIT_VAR(_6);
			ZVAL_STRING(&_6, "\"", 0);
			ZEPHIR_CALL_FUNCTION(&_7, "addcslashes", NULL, 136, defaultValue, &_6);
			zephir_check_call_status();
			ZEPHIR_INIT_VAR(_8);
