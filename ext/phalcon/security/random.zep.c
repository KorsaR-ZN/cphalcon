...
PHP_METHOD(Phalcon_Security_Random, bytes) {
	...
	if ((zephir_function_exists_ex(SS("openssl_random_pseudo_bytes") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_INIT_NVAR(_0);
		ZVAL_LONG(_0, len);
		ZEPHIR_RETURN_CALL_FUNCTION("openssl_random_pseudo_bytes", NULL, 399, _0);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_SINIT_VAR(_1);
	...
	if ((zephir_file_exists(&_1 TSRMLS_CC) == SUCCESS)) {
		...
		ZEPHIR_SINIT_VAR(_3);
		ZVAL_STRING(&_3, "rb", 0);
		ZEPHIR_CALL_FUNCTION(&handle, "fopen", NULL, 278, &_2, &_3);
		zephir_check_call_status();
		if (!ZEPHIR_IS_FALSE_IDENTICAL(handle)) {
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, 0);
			ZEPHIR_CALL_FUNCTION(NULL, "stream_set_read_buffer", NULL, 402, handle, &_2);
			zephir_check_call_status();
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, len);
			ZEPHIR_CALL_FUNCTION(&ret, "fread", NULL, 387, handle, &_2);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Security_Random, uuid) {
	...
	ZEPHIR_CALL_FUNCTION(&_3, "unpack", NULL, 403, &_2, _0);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&ary, "array_values", NULL, 208, _3);
	zephir_check_call_status();
	...
	Z_SET_ISREF_P(ary);
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 377, ary, _7);
...
PHP_METHOD(Phalcon_Security_Random, number) {
	...
	do {
		...
		ZVAL_LONG(&_15, 1);
		ZEPHIR_CALL_FUNCTION(&_16, "substr_replace", &_17, 406, rnd, _11, &_14, &_15);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(rnd, _16);
	} while (ZEPHIR_LT(bin, rnd));
	ZEPHIR_SINIT_NVAR(_10);
	ZVAL_STRING(&_10, "H*", 0);
	ZEPHIR_CALL_FUNCTION(&ret, "unpack", NULL, 403, &_10, rnd);
	zephir_check_call_status();
	Z_SET_ISREF_P(ret);
	ZEPHIR_CALL_FUNCTION(&_11, "array_shift", NULL, 120, ret);
	Z_UNSET_ISREF_P(ret);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("hexdec", NULL, 407, _11);
	zephir_check_call_status();
	RETURN_MM();
