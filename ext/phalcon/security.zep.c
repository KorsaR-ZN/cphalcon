...
PHP_METHOD(Phalcon_Security, getSaltBytes) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&_2, "openssl_random_pseudo_bytes", &_3, 399, _1);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_4, "base64_encode", &_5, 113, _2);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Security, hash) {
	...
	do {
		...
		if (hash == 9) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("crypt", &_4, 400, password, _3);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZEPHIR_INIT_NVAR(_1);
		...
		ZEPHIR_CALL_FUNCTION(&_7, "sprintf", NULL, 181, &_5, &_6);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Security, computeHmac) {
	...
	ZEPHIR_CALL_FUNCTION(&hmac, "hash_hmac", NULL, 401, algo, data, key, (raw ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)));
	zephir_check_call_status();
	if (!(zephir_is_true(hmac))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_0, "phalcon/security.zep", 447 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_CCTOR(hmac);
