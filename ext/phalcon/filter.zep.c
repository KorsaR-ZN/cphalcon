...
PHP_METHOD(Phalcon_Filter, _sanitize) {
	...
	do {
		if (ZEPHIR_IS_STRING(filter, "email")) {
			...
			ZEPHIR_CALL_FUNCTION(&_4, "constant", NULL, 184, &_3);
			zephir_check_call_status();
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 185, value, _4);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "int")) {
			...
			ZVAL_LONG(&_3, 519);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 185, value, &_3);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "int!")) {
		...
		if (ZEPHIR_IS_STRING(filter, "absint")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("abs", NULL, 186, &_3);
			zephir_check_call_status();
			RETURN_MM();
		}
		...
		if (ZEPHIR_IS_STRING(filter, "float")) {
			...
			ZEPHIR_SINIT_NVAR(_3);
			ZVAL_LONG(&_3, 520);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_5, 185, value, &_3, _2);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "float!")) {
		...
		if (ZEPHIR_IS_STRING(filter, "striptags")) {
			ZEPHIR_RETURN_CALL_FUNCTION("strip_tags", NULL, 187, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "lower")) {
			if ((zephir_function_exists_ex(SS("mb_strtolower") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 188, value);
				zephir_check_call_status();
				RETURN_MM();
			}
		...
		if (ZEPHIR_IS_STRING(filter, "upper")) {
			if ((zephir_function_exists_ex(SS("mb_strtoupper") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtoupper", NULL, 189, value);
				zephir_check_call_status();
				RETURN_MM();
			}
