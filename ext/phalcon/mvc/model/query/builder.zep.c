...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, forUpdate) {
	...
	zephir_update_property_this(this_ptr, SL("_forUpdate"), forUpdate ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false) TSRMLS_CC);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Mvc_Model_Query_Builder, getPhql) {
	...
	if (zephir_is_numeric(conditions)) {
		...
		if (zephir_has_constructor(modelInstance TSRMLS_CC)) {
			ZEPHIR_INIT_NVAR(_1);
			ZVAL_NULL(_1);
			ZEPHIR_CALL_METHOD(NULL, modelInstance, "__construct", NULL, 0, _1, dependencyInjector);
			zephir_check_call_status();
		}
		noPrimary = 1;
