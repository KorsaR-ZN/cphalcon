...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC))) {
		...
		if (zephir_is_true(_1)) {
			...
			ZEPHIR_INIT_VAR(_2);
			ZVAL_LONG(_2, 1);
			ZEPHIR_CALL_METHOD(NULL, priorityQueue, "setextractflags", NULL, 178, _2);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, priorityQueue, "insert", NULL, 179, handler, _2);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, _0, eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(priorityQueue) == IS_OBJECT) {
			...
			ZEPHIR_INIT_VAR(_1);
			ZVAL_LONG(_1, 1);
			ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "setextractflags", NULL, 178, _1);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(_1);
			ZVAL_LONG(_1, 3);
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(_5, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, newPriorityQueue, "insert", &_8, 179, _6, _7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), eventType, newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 180, handler, priorityQueue, ZEPHIR_GLOBAL(global_true));
			zephir_check_call_status();
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3, "sprintf", NULL, 181, &_2, _1);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (zephir_array_isset_fetch(&fireEvents, events, type, 0 TSRMLS_CC)) {
		...
		if (_3) {
			...
			ZEPHIR_CALL_METHOD(NULL, event, "__construct", &_4, 182, eventName, source, data, (cancelable ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)));
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", &_5, 183, fireEvents, event);
			zephir_check_call_status();
		}
	}
