...
PHP_METHOD(Phalcon_Debug, listenExceptions) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 143, _0);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, listenLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "set_exception_handler", NULL, 143, _2);
	zephir_check_call_status();
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Debug, debugVar) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "debug_backtrace", NULL, 145);
	zephir_check_call_status();
	zephir_array_fast_append(_0, _1);
	ZEPHIR_INIT_VAR(_2);
	zephir_time(_2);
...
PHP_METHOD(Phalcon_Debug, _escapeString) {
	...
	if (Z_TYPE_P(value) == IS_STRING) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("htmlentities", NULL, 146, _0, &_3, &_4);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		if (Z_TYPE_P(v) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_4, this_ptr, "_getarraydump", &_7, 147, v, _6);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, _getVarDump) {
	...
	if (Z_TYPE_P(variable) == IS_ARRAY) {
		ZEPHIR_CALL_METHOD(&_0, this_ptr, "_getarraydump", &_1, 147, variable);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, getMajorVersion) {
	...
	ZEPHIR_CALL_CE_STATIC(&_0, phalcon_version_ce, "get", &_1, 148);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, getVersion) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_CE_STATIC(&_1, phalcon_version_ce, "get", &_2, 148);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, showTraceItem) {
	...
	if (zephir_array_isset_string_fetch(&className, trace, SS("class"), 0 TSRMLS_CC)) {
		...
		if (zephir_is_true(_3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8, classReflection, "isinternal", NULL, 149);
			zephir_check_call_status();
	...
	if (zephir_array_isset_string(trace, SS("class"))) {
		...
		if ((zephir_function_exists(functionName TSRMLS_CC)  == SUCCESS)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&_8, functionReflection, "isinternal", NULL, 151);
			zephir_check_call_status();
			if (zephir_is_true(_8)) {
	...
	if (zephir_array_isset_string_fetch(&filez, trace, SS("file"), 0 TSRMLS_CC)) {
		...
		if (zephir_is_true(showFiles)) {
			ZEPHIR_CALL_FUNCTION(&lines, "file", NULL, 152, filez);
			zephir_check_call_status();
			...
			while (1) {
				...
				if (_24) {
					...
					ZEPHIR_CALL_FUNCTION(&_8, "htmlentities", &_26, 146, _21, &_23, &_25);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtLowSeverity) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "error_reporting", NULL, 153);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_1);
	...
	if (zephir_is_true(&_1)) {
		...
		ZEPHIR_INIT_VAR(_3);
		ZVAL_LONG(_3, 0);
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, 154, message, _3, severity, file, line);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Debug, onUncaughtException) {
	...
	ZEPHIR_CALL_FUNCTION(&obLevel, "ob_get_level", NULL, 155);
	zephir_check_call_status();
	...
	if (zephir_is_true(showBackTrace)) {
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_11, &_10)
			...
			ZEPHIR_CALL_METHOD(&_13, this_ptr, "showtraceitem", &_14, 156, n, traceItem);
			zephir_check_call_status();
		...
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_16, &_15)
			...
			if (Z_TYPE_P(value) != IS_ARRAY) {
				...
				ZEPHIR_CALL_FUNCTION(&_13, "print_r", &_19, 157, value, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
				ZEPHIR_INIT_LNVAR(_18);
		...
		ZEPHIR_CALL_FUNCTION(&_13, "get_included_files", NULL, 158);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_13, "memory_get_usage", NULL, 159, ZEPHIR_GLOBAL(global_true));
		zephir_check_call_status();
		ZEPHIR_INIT_LNVAR(_27);
