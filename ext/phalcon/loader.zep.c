...
PHP_METHOD(Phalcon_Loader, register) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(_0)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "spl_autoload_register", NULL, 279, _1);
...
PHP_METHOD(Phalcon_Loader, unregister) {
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(_0)) {
		...
		ZVAL_STRING(_2, "autoLoad", 1);
		zephir_array_fast_append(_1, _2);
		ZEPHIR_CALL_FUNCTION(NULL, "spl_autoload_unregister", NULL, 280, _1);
		zephir_check_call_status();
				...
				if (zephir_is_true(fileName)) {
					...
					for (
					  ...
					  ; zephir_hash_move_forward_ex(_7, &_6)
						...
						ZEPHIR_CALL_FUNCTION(&_11, "is_file", &_12, 281, filePath);
						zephir_check_call_status();
